//
// Created by thedyingkai_ on 2025/4/14.
// @formatter:off
#include <bits/stdc++.h>
...
void solve(){
    ...
    int st=1,ti=n*m/k,cnt=0;
    if(m&1) rep(i,0,n-1) rep(j,0,m-1){
        cout<<st<<' ';
        cout<<st+1<<' ';
        j++;
        cnt++;
        if(cnt==ti) cnt=0,st+=2;
    }
    rep(i,0,n-1) rep(j,0,m-1){
        map[i][j]=st;
        if(j!=m-1) map[i][++j]=st+1;
        cnt++;
        if(cnt==ti) cnt=0,st+=2;
    }
    rep(i,0,n-1) rep(j,0,m-1) cout<<map[i][j]<<' '<<endl[j==m-1];
}
