...
void solve(){
    ...
    unsigned ll x;
    cin>>n>>k>>x;
    vector<unsigned ll> a(n+1);
    rep(i,1,n) cin>>a[i];
    vector<unsigned ll> pre(n+1);
    rep(j,1,n) pre[j]=pre[j-1]+a[j];
    unsigned ll res=0;
    rep(i,1,n){
        unsigned ll suf=pre[n]-pre[i-1];
        unsigned ll diff=max(x-suf,0ULL);
        unsigned ll r=(diff+pre[n]-1)/pre[n];
        if(r<(unsigned ll)k) res+=(k-r);
    }
    cout<<res<<endl;
}
